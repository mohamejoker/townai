...
const EnhancedAIChat: React.FC<EnhancedAIChatProps> = ({ onActionClick }) => {
  ...
  const analyzeAccountFromLink = async (link: string) => {
    ...
    const analysisMessage: Message = {
      ...
      actionButtons: [
        {
          label: "🚀 احصل على متابعين مستهدفين",
          action: "get_followers",
          data: { platform: "instagram", type: "targeted" },
        },
        {
          label: "❤️ زيادة التفاعل",
          action: "boost_engagement",
          data: { platform: "instagram" },
        },
        {
          label: "📹 خدمات الريلز",
          action: "reels_services",
          data: { platform: "instagram" },
        },
      ],
    };
  ...
  const generateAIResponse = async (userMessage: string) => {
    ...
    if (userMessage.includes("تحليل") && userMessage.includes("حساب")) {
      response =
        "🔍 ممتاز! لتحليل حسابك بدقة، أرسل لي رابط حسابك على أي منصة.\n\nسأقوم بتحليل:\n• معدل التفاعل والنمو\n• جودة المتابعين\n• أفضل أوقات النشر\n• نوع المحتوى الأكثر نجاحاً\n• مقارنة مع المنافسين\n• اقتراحات تحسين مخصصة";
      suggestions = [
        ...
        <CardTitle className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-blue-100 text-sm">
              مدعوم بالذكاء الاصطناعي • نشط الآن
            </p>
          </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          {message.actionButtons.map((button, index) => (
                            <Button
                              key={index}
                              onClick={() =>
                                handleActionClick(button.action, button.data)
                              }
